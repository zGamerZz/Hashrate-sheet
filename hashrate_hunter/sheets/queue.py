...
def flush_sheet_queue_with_rate_limit(state: StateStore, contexts: Dict[int, SheetContext], write_limiter: TokenBucket) -> int:
    ...
    while i < len(ops):
        ...
        if op_type == "append_round":
        ...
        elif op_type == "append_clan_cpu_round":
            i += 1
            ws_id = int(op["sheet_id"])
            ctx = contexts.get(ws_id)
            if not ctx:
            ...
            try:
                ...
                rows_payload = payload.get("rows")
                if not isinstance(rows_payload, list) or not rows_payload:
                    log_warn("queue.clan_cpu_append_drop_invalid_payload", sheet=ctx.title, op_id=op["id"], round_id=rid)
                    state.delete_op(int(op["id"]))
                    continue
                rows: List[List[Any]] = []
                for row in rows_payload:
                    ...
                    rows.append(norm)
                if not rows:
                    log_warn("queue.clan_cpu_append_drop_invalid_rows", sheet=ctx.title, op_id=op["id"], round_id=rid)
                    state.delete_op(int(op["id"]))
                    continue
                if DRY_RUN:
                    log_info("queue.clan_cpu_append_dry_skip", sheet=ctx.title, round_id=rid, rows=len(rows))
                    continue
                ...
                if not _ensure_sheet_row_capacity(ctx, end_row, write_limiter):
                    _reschedule(state, int(op["id"]), int(op["retry_count"]) + 1, retry_after=30.0)
                    log_warn(
                        "queue.clan_cpu_append_capacity_pending",
                        sheet=ctx.title,
                        ws_id=ws_id,
                        round_id=rid,
                        required_last_row=end_row,
                ...
                if DROP_NON_RETRYABLE_SHEET_ERRORS and not retryable:
                    log_warn(
                        "queue.clan_cpu_append_drop_non_retryable",
                        sheet=ctx.title,
                        ws_id=ws_id,
                        op_id=op_id,
                        round_id=op.get("round_id"),
